#!/bin/bash
#
# VCFA org-provisioning phase of the VCD/vApp use case - ported from the
# reference project's vcf-automation/configure_vcfa.sh, and split out of
# this project's own vcf_bootstrap.sh (which calls this as its final
# phase, once the SDDC build + NSX/Avi/Supervisor setup ahead of it is
# complete) into its own script, matching that reference convention.
...
# "bash configure_vcfa.sh" invocation, not sourced into vcf_bootstrap.sh's
# shell, and log_file is the one exception deliberately kept identical
# between the two (both append to the same /home/ubuntu/vcf_bootstrap.log
