#!/bin/bash
#
# Avi Controller deployment via SDDC Manager, full Avi configuration (NSX cloud integration, SEG, VIP, DNS virtual service), and Avi upgrade.
#
jsonFile="${1}"
...
# app.vcf9.lab, used by anything not org-scoped - Harbor's own FQDN in
...
# vcf_a_organizations is already the full expanded per-org list (empty
# array if sddc.vcf_a is unset, matching every other vcf_a_* variable).
# Confirmed live (pass_through: true on every entry) via the equivalent
# manual config on the sddc/vCenter reference environment's own Avi
# controller.
#
avi_dns_domains_json=$(jq -n --arg base "${avi_subdomain}.${domain}" --argjson orgs "${vcf_a_organizations}" --arg domain "${domain}" \
  '[{domain_name: $base, pass_through: true}]
   + ($orgs | map({domain_name: (.name + "." + $domain), pass_through: true}))
   + ($orgs | map({domain_name: (.name + "-vks." + $domain), pass_through: true}))')
