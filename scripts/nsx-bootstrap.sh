#!/bin/bash
#
# NSX Manager readiness check, uplink profiles/transport zones/segments/host-transport-node-profile, OVF validation flag, and edge node deployment (includes its own small vCenter lookups - vc_id/storage_id/network/cluster/compute_id - self-contained rather than depending on vcenter-bootstrap.sh's separate process).
#
jsonFile="${1}"
